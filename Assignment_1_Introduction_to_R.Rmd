...
```{r, eval = TRUE, comment=""}
lst <- list(4, 'j', 'Y', 5, 'f', 'K', 8, 'z', 'T', 'c', 0, 'B', 2, 'm', 
            'X', 9, 'd', 'V', 3, 'n', 'G', 6, 'h', 'J', 2, 'p', 'O', 1, 'q', 
            'R', 7, 's', 'D', 2, 'l', 'N', 0, 'w', 'E', 5, 'e', 'U', 4, 'i', 
            'P', 1, 'a', 'H', 8, 'y', 'S', 3, 'g', 'F', 9, 'b', 'M', 6, 'k', 'L', 7)
...
my_first_function <- function(vec,x) {
  for (val in vec) {
    if (val <= x) {
      ltoe5_vec <- append(ltoe5_vec, values = val)
    }
    else{
      gt5_vec <- append(gt5_vec, values = val)
    }
  }
  return(list(ltoe_vector = ltoe5_vec, gtoe_vector = gt5_vec))
}

# Test the function
return_vectors <- my_first_function(my_vector,5)

# print(return_vectors[1])
#print(gt5_vec)
